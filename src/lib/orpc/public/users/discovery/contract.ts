...
import {BasicUserSearchOutputSchema, SimilarUsersInputSchema, UserSearchInputSchema} from "../../schemas/users.ts";
...
export const searchByUsernameDisplayContract = oc
  ...
  .route({
    ...
    description: 'Search for users by username with comprehensive display data (use ?format=display query parameter)',
    tags: ['user-discovery'],
...
export const searchSimilarUsersDisplayContract = oc
  ...
  .route({
    ...
    description: 'Discover users with similar profiles or interests based on user ID',
    tags: ['user-discovery'],
