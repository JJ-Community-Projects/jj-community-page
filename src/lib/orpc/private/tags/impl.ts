...
import { checkCanAddTagToUser } from '../util/limits.ts'

const os = implement(privateTagsContract).use(dbMiddleware)
    ...
    try {
      ...
      if (!tag) {
        throw new ORPCError('NOT_FOUND', {
          message: 'Tag not found or not visible',
        })
      }
      ...
      const existingUserTag = await db
        ...
        .where(
          and(
            ...
            eq(userTagsTable.tagId, input.tagId),
          ),
        )
        .get()
      ...
      const tagLimit = await checkCanAddTagToUser(db, userId)
      if (!tagLimit.canAdd) {
        throw new ORPCError('FORBIDDEN', {
          message: 'Profile tag limit reached',
          ...tagLimit,
        })
      }

      // Insert new user tag
      const newUserTag = await db
  ...
  .handler(async ({ context }) => {
    ...
    try {
      const result = await db
        ...
        .innerJoin(
          tags,
          and(eq(userTagsTable.tagId, tags.id), eq(tags.visible, true)),
        )
...
const listAvailableTags = os.listAvailableTags
  ...
  .handler(async ({ context, input }) => {
    ...
    try {
      ...
      const result = await db
        ...
        .where(
          whereConditions.length === 1
            ? whereConditions[0]
            : and(...whereConditions),
        ) // WHERE: Apply visibility and category filters
      ...
      throw new ORPCError('INTERNAL_SERVER_ERROR', {
        message: 'Failed to list available tags',
      })
    }
  })
...
const findUsersByTag = os.findUsersByTag
  ...
  .handler(async ({ context, input }) => {
    ...
    try {
      ...
      throw new ORPCError('INTERNAL_SERVER_ERROR', {
        message: 'Failed to find users by tag',
      })
    }
  })
...
const fullTagsSearch = os.fullTagsSearch
  ...
  .handler(async ({ context, input }) => {
    ...
    try {
      ...
      throw new ORPCError('INTERNAL_SERVER_ERROR', {
        message: 'Failed to search tags with categories',
      })
    }
  })
...
// Limit-checks
const canAddTagToUser = os.canAddTagToUser
  .use(authMiddleware)
  .handler(async ({ context }) => {
    const { db, userId } = context
    return checkCanAddTagToUser(db, userId)
  })
