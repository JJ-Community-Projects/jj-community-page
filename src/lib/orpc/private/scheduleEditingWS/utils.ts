...
export async function ensureDraftInitialized(
  ...
  try {
    const canonicalStreams = await db
      .select()
      .from(streamsTable)
      .where(eq(streamsTable.scheduleId, scheduleId))
      .all()
    if (canonicalStreams.length > 0) {
      for (const c of chunk(canonicalStreams, 5)) {
        await db.insert(editStreamsTable).values(
          c.map((s) => ({
            id: s.id,
    ...
    console.log('canonicalStreams', e)
  }
  ...
  try {
    ...
    if (canonicalTags.length > 0) {
      for (const c of chunk(canonicalTags, 5)) {
        await db
  ...
  try {
    ...
    if (canonicalParts.length > 0) {
      for (const c of chunk(canonicalParts, 5)) {
        await db
...
export async function loadDraftSnapshot(
  db: JJDrizzleDatabase,
  scheduleId: number,
): Promise<LoadDraftSnapshotResult> {
...
export async function resolveTag(db: JJDrizzleDatabase, search: string) {
