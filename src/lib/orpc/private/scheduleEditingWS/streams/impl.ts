...
export const addStream = os.addStreamContract
  ...
  .handler(async ({context, input}) => {
    ...
    const existingIds = await db.select({id: editStreamsTable.id})
      ...
      .all()
    const nonNegSet = new Set(existingIds.map((r: any) => Number(r.id)).filter((id) => Number.isFinite(id) && id >= 0))
...
export const updateStream = os.updateStreamContract
  ...
  .handler(async ({context, input}) => {
    ...
    if ((res as any).rowsAffected === 0) throw new Error('Draft stream not found')

    const stub = getScheduleEditingStub(context.env as any, scheduleId)
    await stub.publishStreamUpdated(scheduleId, editorId, {id, patch: allowed})
