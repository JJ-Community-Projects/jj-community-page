...
const os = implement(friendsContract).use(dbMiddleware)
      ...
      const existingFriendship = await db
        ...
        .where(
          or(
            ...
            and(
              eq(friendsTable.fromUserId, toUserId),
        ...
        .where(
          or(
            and(
              ...
              eq(blockedUsers.blockedUser, toUserId),
            ),
            and(
              eq(blockedUsers.userId, toUserId),
              eq(blockedUsers.blockedUser, userId),
            ),
          ),
        )
        .get()
    ...
    const friendship = await db
      ...
      .where(
        or(
          and(
            eq(friendsTable.fromUserId, userId),
            eq(friendsTable.toUserId, friendUserId),
          ),
          and(
            eq(friendsTable.fromUserId, friendUserId),
            eq(friendsTable.toUserId, userId),
          ),
        ),
      )
...
const getFriends = os.getFriendsContract
  ...
  .handler(async ({ context }) => {
    ...
    const friendships = await db
      ...
      .where(eq(friendsTable.toUserId, userId))
      .all()
