...
import type {
  LegacyRelayResponse1,
  NotFound,
  Unauthorized,
  UnprocessableEntity,
} from '../models/index';
import {
    LegacyRelayResponse1FromJSON,
    LegacyRelayResponse1ToJSON,
    NotFoundFromJSON,
    NotFoundToJSON,
...
export interface GetLegacyRelayRequest {
    provider: string;
    uuid: string;
}
 ...
 * 
 */
export class LegacyRelayApi extends runtime.BaseAPI {

    /**
     * Queries a donation relay
     * @deprecated
     */
    async getLegacyRelayRaw(requestParameters: GetLegacyRelayRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<LegacyRelayResponse1>> {
        if (requestParameters['provider'] == null) {
            throw new runtime.RequiredError(
                'provider',
                'Required parameter "provider" was null or undefined when calling getLegacyRelay().'
            );
        }
        ...
        if (requestParameters['uuid'] == null) {
            throw new runtime.RequiredError(
                'uuid',
                'Required parameter "uuid" was null or undefined when calling getLegacyRelay().'
            );
        }
        ...
        const response = await this.request({
            path: `/api/public/legacy-relays/{provider}/{uuid}`.replace(`{${"provider"}}`, encodeURIComponent(String(requestParameters['provider']))).replace(`{${"uuid"}}`, encodeURIComponent(String(requestParameters['uuid']))),
            method: 'GET',
            headers: headerParameters,
            query: queryParameters,
        }, initOverrides);

        return new runtime.JSONApiResponse(response, (jsonValue) => LegacyRelayResponse1FromJSON(jsonValue));
    }
     ...
     * @deprecated
     */
    async getLegacyRelay(requestParameters: GetLegacyRelayRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<LegacyRelayResponse1> {
        const response = await this.getLegacyRelayRaw(requestParameters, initOverrides);
