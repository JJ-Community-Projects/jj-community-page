 ...
 */
export const tiltifyMetadataView = sqliteView('tiltify_metadata_view').as(
  (qb) => {
    return qb
      .select({
        ...
        // Avatar
        avatarSrc:
          sql<string>`json_extract(${accounts.meta}, '$.avatar.src')`.as(
            'avatar_src'
          ),
        // avatarAlt: sql<string>`json_extract(${accounts.meta}, '$.avatar.alt')`.as('avatar_alt'),
        // Basic info
        description:
          sql<string>`json_extract(${accounts.meta}, '$.description')`.as(
            'description'
          ),
        ...
        username: sql<string>`json_extract(${accounts.meta}, '$.username')`.as(
          'username'
        ),
        // Social links
        /*
 ...
 */
export const userDisplayView = sqliteView('user_display_view').as((qb) => {
  return qb
    .select({
      userId: users.id,
 ...
 */
export const usersSearchView = sqliteView('users_search_view').as((qb) => {
  return qb
    .select({
      userId: users.id,
      tiltifyUsername: sql<string>`lower(${tiltifyMetadataView.username})`.as(
        'tiltifyUsername'
      ),
      twitchUsername: sql<string>`lower(${twitchChannelSchema.displayName})`.as(
        'twitchUsername'
      ),
    })
    .from(users)
...
export const streamParticipantsUIView = sqliteView(
  'stream_participants_ui_view'
).as((qb) => {
  return qb
    .select({
    ...
    .leftJoin(userStyles, eq(streamParticipantsTable.userId, userStyles.userId))
})

export const socialNames = sqliteView('social_names_view').as((qb) => {
  return qb
    .select({
      userId: userSocials.userId,
      provider: userSocials.provider,
      url: userSocials.url,
      name: sql`substr(url, length(url) - instr(reverse(url), '/') + 2)`.as(
        'name'
      ),
    })
    .from(userSocials)
 ...
 */
export const streamParticipantsDisplayView = sqliteView(
  'stream_participants_display_view'
).as((qb) => {
  return qb
    .select({
      streamId: streamParticipantsTable.streamId,
