...
import {useParam} from '../overlay_signals.ts';
import {SlotUtils} from "./slotUtils.ts";
import img from '../../../images/scheduleOverlay/Jingle_Jam_Background_Greyscale_2.png'
...
const OverlayScheduleSlotV2: Component<MobileScheduleSlotProps> = props => {
  ...
  const background = () => {
    let gradientStyle: any | undefined
  ...
  const countdown = () => {
    ...
    return start.diff(useOverlayNow()).toFormat('hh:mm:ss')
  }

  const showLiveText = () => {
    const now = date()
    const start = DateTime.fromISO(slot.start)
    const end = start.plus(Duration.fromDurationLike({second: slot.duration}))
