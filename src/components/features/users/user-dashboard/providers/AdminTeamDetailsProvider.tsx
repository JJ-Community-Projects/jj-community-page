...
const useAdminTeamDetailsHook = (teamId: number, user: User) => {
  ...
  const membersQuery = useQuery(() =>
    teams.getTeamMembers.queryOptions({
      input: {teamId},
      staleTime: 30 * 1000,
    })
  );
  ...
  const memberCountQuery = useQuery(() =>
    teams.getTeamMemberCount.queryOptions({
      input: {teamId},
      staleTime: 30 * 1000,
    })
  );
  ...
  const inviteCountQuery = useQuery(() =>
    teams.getTeamInviteCount.queryOptions({
      input: {teamId},
      staleTime: 30 * 1000,
    })
  );

  const inviteQuery = useQuery(() => teams.getTeamInvites.queryOptions({
    input: {teamId},
    staleTime: 30 * 1000,
  }))
  ...
  const cancelInviteMutation = useMutation(() =>
    teams.deleteInvite.mutationOptions({
      onSuccess: async () => {
        await queryClient.invalidateQueries({queryKey: teams.getTeamInviteCount.queryKey({input: {teamId}})});
      },
    })
  );
...
export const AdminTeamDetailsProvider: ParentComponent<AdminTeamDetailsProps> = (props) => {
  const hook = useAdminTeamDetailsHook(props.teamId, props.user);
  return (
