...
const useUserFriendsHook = () => {
  ...
  const hasDeclineFriendRequestError = () =>
  ...
  const friendRequestsErrorMessage = () =>
    friendRequestsQuery.error?.message || 'Failed to load friend requests'
  const sentFriendRequestsErrorMessage = () =>
    sentFriendRequestsQuery.error?.message ||
    'Failed to load sent friend requests'
  const friendsErrorMessage = () =>
    friendsListQuery.error?.message || 'Failed to load friends'

  // Mutation error message helpers
  const sendFriendRequestErrorMessage = () =>
    sendFriendRequestMutation.error?.message || 'Failed to send friend request'
  const acceptFriendRequestErrorMessage = () =>
    acceptFriendRequestMutation.error?.message ||
    'Failed to accept friend request'
  const declineFriendRequestErrorMessage = () =>
    declineFriendRequestMutation.error?.message ||
    'Failed to decline friend request'
  const cancelFriendRequestErrorMessage = () =>
    cancelFriendRequestMutation.error?.message ||
    'Failed to cancel friend request'
  const removeFriendErrorMessage = () =>
    removeFriendMutation.error?.message || 'Failed to remove friend'
...
const UserFriendsContext =
  createContext<ReturnType<typeof useUserFriendsHook>>()

export const UserFriendsProvider: ParentComponent<UserFriendsProps> = (
  props,
) => {
